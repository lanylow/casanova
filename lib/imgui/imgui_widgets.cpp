...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(pos, { pos.x + 100.f, pos.y + 25.f });
    const ImRect total_bb(pos, { pos.x + 100.f, pos.y + 25.f });
    ...
    if (is_multiline)
    {
        ...
        ItemSize({ 200.f, 25.f }, style.FramePadding.y);
    ...
    else
    {
        ...
        if (!(flags & ImGuiInputTextFlags_MergedItem))
            if (!ItemAdd(total_bb, id, nullptr, ImGuiItemFlags_Inputable))
                return false;
        item_status_flags = g.LastItemData.StatusFlags;
    }
    const bool hovered = ItemHoverable(frame_bb, id);
    ...
    if (!is_multiline)
    {
        ...
        RenderFrame({ frame_bb.Min.x + 3.f, frame_bb.Min.y + 3.f }, { frame_bb.Max.x - 3.f, frame_bb.Min.y + 22.f }, ImColor(28, 28, 28, 255), true, style.FrameRounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    //ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding;
    ImVec2 draw_pos = { frame_bb.Min.x + 5.f, frame_bb.Min.y + 1.f };
